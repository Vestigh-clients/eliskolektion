...
const AdminInventoryPricingPage = () => {
  ...
  return (
    <div className="admin-page">
      <div className="admin-page-header">
        ...
        <div className="admin-page-actions">
          ...
          <button
            ...
            disabled={!canSave}
            className="rounded-[var(--border-radius)] bg-[var(--color-primary)] px-7 py-3 text-center font-inter text-[11px] uppercase tracking-[0.1em] text-[var(--color-secondary)] transition-colors hover:bg-[var(--color-accent)] hover:text-[var(--color-secondary)] disabled:cursor-not-allowed disabled:opacity-40"
          >
      ...
      {saveMessage ? (
        <p className={`mb-4 font-inter text-[12px] ${saveToneClass(saveTone)}`}>{saveMessage}</p>
      ) : null}
      ...
      <div className="hidden overflow-x-auto md:block">
        <table className="w-full min-w-[760px] border-collapse">
          ...
          <tbody>
            {isLoading ? (
              <tr>
                <td colSpan={3} className="px-0 py-8 text-center font-inter text-[12px] text-[var(--color-muted-soft)]">
                  Loading rows...
                </td>
              </tr>
            ) : loadError ? (
              <tr>
                <td colSpan={3} className="px-0 py-8 text-center font-inter text-[12px] text-[var(--color-danger)]">
                  {loadError}
                </td>
              </tr>
                ...
                >
                    ...
                    <div className="flex min-w-0 items-start gap-3">
                      <div className="h-[52px] w-[40px] shrink-0 overflow-hidden rounded-[10px] bg-[var(--color-surface-alt)]">
                        {state.row.product_image_url ? (
                          ...
                          <div className="flex h-full w-full items-center justify-center bg-[var(--color-surface-strong)] font-inter text-[11px] uppercase tracking-[0.08em] text-[var(--color-muted)]">
                      ...
                      <div className="min-w-0">
                        <p className="break-words font-inter text-[13px] text-[var(--color-primary)]">{state.row.item_label}</p>
                        <div className="mt-1 flex flex-wrap items-center gap-2">
                          <span className="rounded-[var(--border-radius)] border border-[var(--color-border)] px-2 py-0.5 font-inter text-[9px] uppercase tracking-[0.08em] text-[var(--color-muted)]">
                  ...
                  <td className="px-2 py-4 align-top">
                    <input
                      ...
                      className="w-full max-w-[120px] border-0 border-b border-[var(--color-border)] bg-transparent pb-1 font-inter text-[13px] text-[var(--color-primary)] outline-none focus:border-[var(--color-primary)]"
                    />
                    {state.stockError ? (
                  ...
                  <td className="px-0 py-4 align-top">
                    <input
                      ...
                      className="w-full max-w-[180px] border-0 border-b border-[var(--color-border)] bg-transparent pb-1 font-inter text-[13px] text-[var(--color-primary)] outline-none placeholder:text-[var(--color-muted-soft)] focus:border-[var(--color-primary)]"
                    />
                    ...
                    {state.serverError ? (
                      <p className="mt-1 font-inter text-[10px] text-[var(--color-danger)]">{state.serverError}</p>
                    ) : null}
                  </td>
                </tr>
              ))
      ...
      <div className="border-t border-[var(--color-border)] md:hidden">
        {isLoading ? (
          <p className="py-8 text-center font-inter text-[12px] text-[var(--color-muted-soft)]">Loading rows...</p>
        ) : loadError ? (
          <p className="py-8 text-center font-inter text-[12px] text-[var(--color-danger)]">{loadError}</p>
        ) : filteredStates.length === 0 ? (
          <p className="py-8 text-center font-inter text-[12px] text-[var(--color-muted-soft)]">No matching rows found.</p>
        ) : (
          filteredStates.map((state) => (
            <div key={state.row.row_id} className="admin-mobile-card">
              <div className="flex items-start justify-between gap-3">
                <div className="flex min-w-0 flex-1 items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="break-words font-inter text-[12px] text-[var(--color-primary)]">{state.row.item_label}</p>
                    ...
                    <p className="mt-1 break-all font-inter text-[10px] text-[var(--color-muted-soft)]">
                      {state.row.sku ? `SKU: ${state.row.sku}` : "No SKU"}
                    </p>
                  </div>
                </div>
                ...
                {state.dirty ? (
                  <span className="rounded-[var(--border-radius)] border border-[var(--color-accent)] px-2 py-1 font-inter text-[9px] uppercase tracking-[0.08em] text-[var(--color-accent)]">
                    Unsaved
                  </span>
                ) : null}
              </div>
              ...
              <div className="mt-3 grid grid-cols-2 gap-3">
                <label className="block min-w-0">
                  ...
                  <input
                    ...
                    value={state.edit.stock}
                    onChange={(event) => updateRowEdit(state.row.row_id, "stock", event.target.value)}
                    className="mt-1 w-full border-0 border-b border-[var(--color-border)] bg-transparent pb-1 font-inter text-[13px] text-[var(--color-primary)] outline-none focus:border-[var(--color-primary)]"
                  />
                </label>

                <label className="block min-w-0">
                  <span className="font-inter text-[10px] uppercase tracking-[0.08em] text-[var(--color-muted-soft)]">Price</span>
                  <input
                    type="number"
                    min={0}
                    step="0.01"
                    ...
                    className="mt-1 w-full border-0 border-b border-[var(--color-border)] bg-transparent pb-1 font-inter text-[13px] text-[var(--color-primary)] outline-none placeholder:text-[var(--color-muted-soft)] focus:border-[var(--color-primary)]"
                  />
                </label>
              </div>
              ...
              {state.stockError ? <p className="mt-1 font-inter text-[10px] text-[var(--color-danger)]">{state.stockError}</p> : null}
              {state.priceError ? <p className="mt-1 font-inter text-[10px] text-[var(--color-danger)]">{state.priceError}</p> : null}
              {state.serverError ? <p className="mt-1 font-inter text-[10px] text-[var(--color-danger)]">{state.serverError}</p> : null}
            </div>
          ))
        )}
      </div>
