...
const AccountOverview = () => {
  ...
  return (
    <div>
      <h1 className="font-manrope text-[36px]  text-[var(--color-primary)]">Welcome back, {firstName}.</h1>

      <div className="mt-10 grid gap-8 md:grid-cols-3">
        <div className="border-b-2 border-[var(--color-accent)] pb-4">
          <p className="font-manrope text-[32px] text-[var(--color-primary)]">{totalOrders}</p>
          <p className="mt-1 font-inter text-[10px] uppercase tracking-[0.12em] text-[var(--color-muted-soft)]">Total Orders</p>
        </div>

        <div className="border-b-2 border-[var(--color-accent)] pb-4">
          <p className="font-manrope text-[32px] text-[var(--color-primary)]">{formatPrice(totalSpent)}</p>
          <p className="mt-1 font-inter text-[10px] uppercase tracking-[0.12em] text-[var(--color-muted-soft)]">
            Total Spent (GH&#8373;)
          </p>
        </div>

        <div className="border-b-2 border-[var(--color-accent)] pb-4">
          <p className="font-manrope text-[32px] text-[var(--color-primary)]">{formatMemberSince(profile?.created_at)}</p>
          <p className="mt-1 font-inter text-[10px] uppercase tracking-[0.12em] text-[var(--color-muted-soft)]">Member Since</p>
        </div>
      </div>

      <section className="mt-14">
        <div className="mb-4 flex items-center justify-between gap-4">
          <h2 className="font-manrope text-[30px]  text-[var(--color-primary)]">Recent Orders</h2>
          <Link
            to="/account/orders"
            className="font-inter text-[10px] uppercase tracking-[0.12em] text-[var(--color-primary)] transition-colors hover:text-[var(--color-accent)]"
          >
            View All Orders
          </Link>
        </div>
        ...
        {isOrdersLoading ? (
          ...
          <p className="font-inter text-[12px] text-[var(--color-danger)]">{ordersError}</p>
